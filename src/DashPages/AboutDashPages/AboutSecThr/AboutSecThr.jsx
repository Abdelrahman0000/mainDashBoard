...
export default function AboutSecThr() {
  ...
  useEffect(() => {
    if (About) {
      setTitleAr(About?.sec3_text1_ar || ''); // Use empty string as fallback if placeholder value is null
      setTitleEn(About?.sec3_text1_en || '');
      setTextAr(About?.sec3_text2_ar || '');
      setTextEn(About?.sec3_text2_en || '');
    }
  }, [About]);
  ...
  const imageUrl = selectedFile ? URL.createObjectURL(selectedFile) : (About ? About.sec3_image : '');
  ...
  const handleSubmit = async (event) => {
    ...
    if (selectedFile) {
      formData.append('sec3_image', selectedFile);
    }
    ...
    try {
     await fetch('https://bluedana.mass-fluence.com/api/update-about', {
      ...
      console.error('Error updating sec3:', error);
    ...
    <div className={`home`}>
     {About&&    <form onSubmit={handleSubmit}>
        <div className="my-row">
          <label>
            <p>Title in ar</p>
            <textarea type="text" value={titleAr} placeholder={About?.sec3_text1_ar} onChange={handleTitleArChange} />
          </label>
          <label>
            <p>Title in en</p>
            <textarea type="text" value={titleEn} placeholder={About?.sec3_text1_en} onChange={handleTitleEnChange} />
          </label>
        </div>
        ...
        <div className="my-row">
          <label>
            <p>Text in ar</p>
            <textarea type="text" value={textAr} placeholder={About?.sec3_text2_ar} onChange={handleTextArChange} />
          </label>
          <label>
            <p>Text in en</p>
            <textarea type="text" value={textEn} placeholder={About?.sec3_text2_en} onChange={handleTextEnChange} />
          </label>
        </div>
