...
export default function AboutSecTwo() {
  ...
  useEffect(() => {
    if (About) {
      setTitleAr(About?.sec2_text1_ar || ''); // Use empty string as fallback if placeholder value is null
      setTitleEn(About?.sec2_text1_en || '');
      setTextAr(About?.sec2_text2_ar || '');
      setTextEn(About?.sec2_text2_en || '');
    }
  }, [About]);
  ...
  const imageUrl = selectedFile ? URL.createObjectURL(selectedFile) : (About ? About.sec2_image : '');
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      console.error('Error updating sec2:', error);
    ...
    <div className={`home`}>
     {About&&    <form onSubmit={handleSubmit}>
        <div className="my-row">
          <label>
            <p>Title in ar</p>
            <textarea type="text" value={titleAr} placeholder={About?.sec2_text1_ar} onChange={handleTitleArChange} />
          </label>
          <label>
            <p>Title in en</p>
            <textarea type="text" value={titleEn} placeholder={About?.sec2_text1_en} onChange={handleTitleEnChange} />
          </label>
        </div>
        ...
        <div className="my-row">
          <label>
            <p>Text in ar</p>
            <textarea type="text" value={textAr} placeholder={About?.sec2_text2_ar} onChange={handleTextArChange} />
          </label>
          <label>
            <p>Text in en</p>
            <textarea type="text" value={textEn} placeholder={About?.sec2_text2_en} onChange={handleTextEnChange} />
          </label>
        </div>
