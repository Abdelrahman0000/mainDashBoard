...
export default function Constructionsec1() {
  ...
  useEffect(() => {
    if (Constdata) {
      setTitleAr(Constdata?.sec1_text1_ar || '');
      setTitleEn(Constdata?.sec1_text1_en || '');
      setTextAr(Constdata?.sec1_text2_ar || '');
      setTextEn(Constdata?.sec1_text2_en || '');
    }
  }, [Constdata]);
  ...
  const handleImage2ChangeClick = () => {
    console.log(Constdata.sec1_image2); 
    if (fileInputRef2.current) {
      fileInputRef2.current.click();
    }
  };

  const imageUrl = selectedFile ? URL.createObjectURL(selectedFile) : (Constdata ? Constdata.sec1_image1 : '');
  const image2Url = selectedFile2 ? URL.createObjectURL(selectedFile2) : (Constdata ? Constdata.sec1_image2 : '');
  ...
  const handleSubmit = async (event) => {
    ...
    if (selectedFile) {
      formData.append('sec1_image1', selectedFile);
    }
    if (selectedFile2) {
      formData.append('sec1_image2', selectedFile2);
    }
      ...
      {Constdata && (
        <form onSubmit={handleSubmit}>
          <div className="my-row">
            <label>
              <p>Title in ar</p>
              <textarea type="text" value={titleAr} placeholder={Constdata?.sec1_text1_ar} onChange={handleTitleArChange} />
            </label>
            <label>
              <p>Title in en</p>
              <textarea type="text" value={titleEn} placeholder={Constdata?.sec1_text1_en} onChange={handleTitleEnChange} />
            </label>
          </div>
          ...
          <div className="my-row">
            <label>
              <p>Text in ar</p>
              <textarea type="text" value={textAr} placeholder={Constdata?.sec1_text2_ar} onChange={handleTextArChange} />
            </label>
            <label>
              <p>Text in en</p>
              <textarea type="text" value={textEn} placeholder={Constdata?.sec1_text2_en} onChange={handleTextEnChange} />
            </label>
          </div>
          <div className="my-row">
            <div className="box">
