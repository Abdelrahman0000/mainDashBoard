...
export default function NewsIntro() {
  ...
  const fetchData = async () => {
    const response = await fetch("https://bluedana.mass-fluence.com/api/news");
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await fetch('https://bluedana.mass-fluence.com/api/update-background-news', {
        method: 'POST',
        body: formData,
      });
      window.alert('Update successful!');
    } catch (error) {
      console.error('Error updating News intro:', error);
