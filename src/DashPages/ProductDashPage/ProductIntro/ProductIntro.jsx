...
export default function ProductIntro() {
  ...
  const fetchData = async () => {
    const response = await fetch("https://bluedana.mass-fluence.com/api/all-products-front");
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      await fetch('https://bluedana.mass-fluence.com/api/update-background-product', {
        method: 'POST',
        body: formData,
      });
      window.alert('Update successful!');
    } catch (error) {
      console.error('Error updating Product intro:', error);
